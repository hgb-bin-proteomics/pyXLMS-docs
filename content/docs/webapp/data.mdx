...
- **Validation via FDR Estimation**
  ...
  > [!IMPORTANT]
  >
  > **We recommend uploading results that are already validated in the crosslink search engine**
  > **of your choice. Validation in pyXLMS uses a generic target-decoy approach with a single score**
  > **that might not yield the same results as the search engine specific validation!**
...
- **Validation of Aggregated XLs**
  ...
  via FDR estimation. Shares the same settings as `Validation via FDR Estimation`.
